...
export function UltimateBoard({ state, localPlayer, onMove, animationsEnabled }: UltimateBoardProps) {
  ...
  let lineGeometry: { x1: number; y1: number; x2: number; y2: number } | null = null;
  if (winLine && winLine.length === 3) {
    const [a, , c] = winLine;
    const [r1, c1] = WIN_LINE_COORDS[a];
    const [r2, c2] = WIN_LINE_COORDS[c];
    lineGeometry = {
      x1: (c1 + 0.5) * (100 / 3),
      y1: (r1 + 0.5) * (100 / 3),
      x2: (c2 + 0.5) * (100 / 3),
  ...
  return (
    <div className="relative mx-auto w-full max-w-md">
      <div className="grid grid-cols-3 gap-1.5 rounded-3xl bg-gradient-to-br from-slate-200/80 to-slate-300/50 p-1.5 shadow-inner dark:from-slate-800/80 dark:to-slate-900/50 sm:gap-2 sm:p-2">
        {state.boards.map((board, boardIndex) => (
          <SmallBoardView
            ...
            isPlayable={isMyTurn && legalBoards.includes(boardIndex)}
            onCellClick={(cellIndex) => onMove(boardIndex, cellIndex)}
      ...
      <AnimatePresence>
        {lineGeometry && (
          ...
          >
            <motion.line
              initial={animationsEnabled ? { pathLength: 0 } : false}
